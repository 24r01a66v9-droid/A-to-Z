...
function App() {
  ...
  const navItems = role === 'delivery'
    ...
    : role === 'farmer'
    ? [['farmer', t('farmer'), Sprout], ['add-listing', 'Add produce', Plus], ['inputs', 'Farm inputs', Store], ['connect', retailConnectTranslations[language].nav, MessageCircle], ['radar', t('radar'), Radar], ['orders', t('orders'), Truck], ['reviews', 'Reviews', Star]]
  ...
  return <div className="app-shell">
    ...
    <div className="mobile-nav">{navItems.map(([key, label, Icon]) => <button className={tab === key ? 'active' : ''} key={key} onClick={() => key === 'add-listing' ? setShowAdd(true) : setTab(key)}><Icon size={19} /><span>{label.split(' ')[0]}</span></button>)}</div>
...
function DemandRadar() {
  ...
  return <section className="radar-page page-enter">
    ...
    <div className="farm-conditions"><div className="conditions-heading"><div><div className="eyebrow muted">FIELD CONDITIONS</div><h2>Personalise your farm advice</h2><p>Add your area, weather, and water procedure for a practical harvest plan.</p></div><MapPin size={21} /></div><div className="conditions-fields"><label><span>Farm area</span><select value={farmArea} onChange={(event) => setFarmArea(event.target.value)}>{Object.keys(areaProfiles).map((area) => <option key={area}>{area}</option>)}</select></label><label><span>Current weather</span><select value={weather} onChange={(event) => setWeather(event.target.value)}><option>Sunny</option><option>Cloudy</option><option>Rainy</option><option>Heatwave</option></select></label><label><span>Water procedure</span><select value={waterMethod} onChange={(event) => setWaterMethod(event.target.value)}><option>Drip irrigation</option><option>Sprinkler</option><option>Flood irrigation</option><option>Rainwater harvesting</option></select></label></div><div className="conditions-summary"><div><span>{farmArea} · {selectedArea.soil}</span><strong>{selectedArea.note}</strong></div><div><span>Harvest suggestion</span><strong>{harvestAdvice}</strong></div><div><span>Water suggestion</span><strong>{waterAdvice}</strong></div></div></div>
