...
function App() {
  ...
  useEffect(() => {
    ...
    const loadProfile = async (session) => {
      ...
      const response = await fetch(`${DELIVERY_API_URL}/api/me`, { headers: { Authorization: `Bearer ${session.access_token}` } });
      if (!response.ok) { await supabase.auth.signOut(); if (active) { setCurrentUser(null); setRole('consumer'); setAuthLoading(false); } return; }
    ...
    const { data: subscription } = supabase.auth.onAuthStateChange((_event, session) => { void loadProfile(session); });
  ...
  const navItems = role === 'delivery_partner'
    ...
    : role === 'farmer'
    ? [['farmer', t('farmer'), Sprout], ['add-listing', t('addProduce'), Plus], ['inputs', t('farmInputs'), Store], ['radar', t('radar'), Radar], ['orders', t('previousOrders'), Truck], ['reviews', t('reviews'), Star]]
    : [['market', householdLabels[language] || householdLabels.en, Store], ['orders', t('previousOrders'), Truck], ['reviews', t('reviews'), Star]];
...
function Marketplace({ produce, query, setQuery, category, setCategory, buyerType, setBuyerType, showBuyerType, onSelect, onAddToCart }) {
...
function RetailConnect({ produce }) {
  ...
  const farmers = Object.values(produce.filter((item) => item.category === 'Vegetables' && item.quantity > 0).reduce((groups, item) => {
    const current = groups[item.farmer] || { id: item.farmer, farmer: item.farmer, farm: item.farm, location: item.location, color: item.color, farmType: item.organic ? 'Organic' : 'Conventional', produce: [] };
...
function AddListing({ onClose, onAdd, language = 'en', initialName = '' }) {
  const [form, setForm] = useState({ name: initialName, category: 'Vegetables', farmPrice: '', middlemanPrice: '', unit: 'kg', quantity: '', farmer: '', farmerPhone: '', farm: '', location: '', organic: true, productionMethod: 'Organic', harvest: '', description: '', photo: '' });
  ...
  return <div className="modal-backdrop"><form className="modal add-modal detailed-form" onSubmit={submit}>
    ...
    <div className="form-grid">
      <label className="field"><span>{labels.farmPrice}</span><input required type="number" min="1" value={form.farmPrice} onChange={(event) => update('farmPrice', event.target.value)} placeholder="22" /></label>
      <label className="field"><span>{labels.marketPrice}</span><input required type="number" min="1" value={form.middlemanPrice} onChange={(event) => update('middlemanPrice', event.target.value)} placeholder="45" /></label>
      <label className="field"><span>{labels.quantity}</span><input required type="number" min="1" step="0.1" value={form.quantity} onChange={(event) => update('quantity', event.target.value)} placeholder="150" /></label>
    ...
    <div className="form-grid">
      <label className="field"><span>{labels.farmer}</span><input required value={form.farmer} onChange={(event) => update('farmer', event.target.value)} placeholder="Your full name" /></label>
      <label className="field"><span>{labels.phone}</span><input required type="tel" value={form.farmerPhone} onChange={(event) => update('farmerPhone', event.target.value)} placeholder="+91 98765 43210" /></label>
      <label className="field"><span>{labels.farmName}</span><input required value={form.farm} onChange={(event) => update('farm', event.target.value)} placeholder="Your farm or co-op" /></label>
      <label className="field wide"><span>{labels.location}</span><input required value={form.location} onChange={(event) => update('location', event.target.value)} placeholder="Village, district, state" /></label>
      <label className="field wide"><span>{labels.harvest}</span><input value={form.harvest} onChange={(event) => update('harvest', event.target.value)} placeholder="Harvested today, sun-dried last week..." /></label>
      <label className="field wide"><span>{labels.description}</span><textarea rows="3" value={form.description} onChange={(event) => update('description', event.target.value)} placeholder="Tell buyers how it is grown or what makes it special." /></label>
...
function AuthModal({ mode, onModeChange, onClose, onAuthenticated }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (isReset) { const { error: resetError } = await supabase.auth.resetPasswordForEmail(normalizedEmail); if (resetError) throw resetError; setMessage('Check your email for a password reset link.'); return; }
      if (mode === 'register') { const { data, error: signUpError } = await supabase.auth.signUp({ email: normalizedEmail, password, options: { data: { name: name.trim(), role: registerRole, buyer_type: buyerType } } }); if (signUpError) throw signUpError; if (!data.session) { setMessage('Account created. Confirm your email before signing in.'); return; } }
      else { const { error: signInError } = await supabase.auth.signInWithPassword({ email: normalizedEmail, password }); if (signInError) throw signInError; }
      await supabase.auth.updateUser({ data: { buyer_type: buyerType } });
      const session = await supabase.auth.getSession(); const token = session.data.session?.access_token; const response = await fetch(`${DELIVERY_API_URL}/api/me`, { headers: { Authorization: `Bearer ${token}` } }); const result = await response.json(); if (!response.ok) throw new Error(result.error || 'Your account is not approved.'); if (mode === 'login' && result.profile.role !== loginRole) throw new Error(`This account is registered as ${result.profile.role.replace('_', ' ')}.`); onAuthenticated({ ...result.user, ...result.profile, buyerType });
  ...
  return <div className="modal-backdrop"><div className="modal auth-modal">
    ...
    <div className="auth-panel"><div className="auth-icon"><KeyRound size={25} /></div><div className="eyebrow"><span></span> FARMERS A TO Z</div><h2>{title}</h2><p>{isReset ? 'We will prepare a password reset request for your email address.' : mode === 'register' ? 'Create a Consumer, Farmer, or Delivery Partner account. Customer Care access is invitation-only, and Admin access is created manually by the owner.' : 'Keep your orders, savings, and farm connections together.'}</p></div>
    <form className="auth-form" onSubmit={submit}>
      ...
      {!isReset && mode === 'login' && <label className="field"><span>Login as</span><select value={loginRole} onChange={(event) => setLoginRole(event.target.value)}><option value="consumer">Consumer</option><option value="farmer">Farmer</option><option value="delivery_partner">Delivery Partner</option><option value="customer_care">Customer Care</option><option value="admin">Admin</option></select></label>}
      {!isReset && (registerRole === 'consumer' || (mode === 'login' && loginRole === 'consumer')) && <label className="field"><span>Buying for</span><select value={buyerType} onChange={(event) => setBuyerType(event.target.value)}><option>Household</option><option>Caterer</option><option>Supermarket</option><option>Retailer</option></select></label>}
      {error && <p className="auth-error">{error}</p>}{message && <p className="auth-success">{message}</p>}
      <button className="primary-button full-button" type="submit" disabled={busy}>{isReset ? <><Mail size={16} /> Send reset link</> : mode === 'register' ? <><LogIn size={16} /> Create account</> : <><LogIn size={16} /> Log in</>}</button>
...
createRoot(document.getElementById('root')).render(<><LanguageProvider><App /></LanguageProvider><WhatsAppSupport /></>);